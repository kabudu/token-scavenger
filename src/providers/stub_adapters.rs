...
use crate::providers::http::{bearer_auth, custom_auth, ProviderHttp};
...
impl ProviderAdapter for OpenRouterAdapter {
    ...
    fn display_name(&self) -> &'static str { "OpenRouter (Free)" }
    ...
    fn capabilities(&self) -> ProviderCapabilities {
        ProviderCapabilities {
            ...
            has_quirks: true,
            quirks: vec!["Free models use :free suffix".into(), "Supports model fallback via models[] array".into()],
    ...
    async fn embeddings(&self, _ctx: &ProviderContext, _request: NormalizedEmbeddingsRequest) -> Result<ProviderEmbeddingsResponse, ProviderError> {
        Err(ProviderError::UnsupportedFeature("OpenRouter does not support embeddings".into()))
    }
...
impl ProviderAdapter for CerebrasAdapter {
    fn provider_id(&self) -> &'static str { "cerebras" }
    fn display_name(&self) -> &'static str { "Cerebras" }
    fn supports_endpoint(&self, kind: &EndpointKind) -> bool {
        matches!(kind, EndpointKind::ChatCompletions | EndpointKind::ModelList)
    }
    fn auth_kind(&self) -> AuthKind { AuthKind::Bearer }
    fn capabilities(&self) -> ProviderCapabilities {
        ProviderCapabilities {
            openai_compatible: true, has_quirks: true,
            quirks: vec!["Response includes extra time_info field".into(), "Rate limits per model: 30 RPM / 64K TPM".into()],
            supports_streaming: true, supports_tools: true, supports_json_mode: true, supports_vision: false,
            docs_url: Some("https://inference-docs.cerebras.ai".into()),
        }
    }
    fn base_url(&self, config: &ProviderConfig) -> Url {
        config.base_url.as_ref().map(|u| u.parse().unwrap()).unwrap_or_else(|| "https://api.cerebras.ai/v1".parse().unwrap())
    }
    fn default_headers(&self, config: &ProviderConfig) -> HeaderMap { bearer_auth(config) }
    async fn discover_models(&self, ctx: &ProviderContext) -> Result<Vec<DiscoveredModel>, ProviderError> { shared::openai_discover_models(ctx, "cerebras").await }
    async fn chat_completions(&self, ctx: &ProviderContext, request: NormalizedChatRequest) -> Result<ProviderChatResponse, ProviderError> { shared::openai_chat_completions(ctx, request, "cerebras").await }
    async fn embeddings(&self, _ctx: &ProviderContext, _request: NormalizedEmbeddingsRequest) -> Result<ProviderEmbeddingsResponse, ProviderError> {
        Err(ProviderError::UnsupportedFeature("Cerebras does not support embeddings".into()))
    }
...
impl ProviderAdapter for NvidiaAdapter {
    fn provider_id(&self) -> &'static str { "nvidia" }
    fn display_name(&self) -> &'static str { "NVIDIA NIM" }
    fn supports_endpoint(&self, kind: &EndpointKind) -> bool {
        matches!(kind, EndpointKind::ChatCompletions | EndpointKind::ModelList)
    }
    fn auth_kind(&self) -> AuthKind { AuthKind::Bearer }
    fn capabilities(&self) -> ProviderCapabilities {
        ProviderCapabilities {
            ...
            supports_streaming: true, supports_tools: true, supports_json_mode: true, supports_vision: true,
            docs_url: Some("https://build.nvidia.com/docs".into()),
        }
    }
    fn base_url(&self, config: &ProviderConfig) -> Url {
        config.base_url.as_ref().map(|u| u.parse().unwrap()).unwrap_or_else(|| "https://integrate.api.nvidia.com/v1".parse().unwrap())
    }
    fn default_headers(&self, config: &ProviderConfig) -> HeaderMap { bearer_auth(config) }
    async fn discover_models(&self, ctx: &ProviderContext) -> Result<Vec<DiscoveredModel>, ProviderError> { shared::openai_discover_models(ctx, "nvidia").await }
    async fn chat_completions(&self, ctx: &ProviderContext, request: NormalizedChatRequest) -> Result<ProviderChatResponse, ProviderError> { shared::openai_chat_completions(ctx, request, "nvidia").await }
    async fn embeddings(&self, _ctx: &ProviderContext, _request: NormalizedEmbeddingsRequest) -> Result<ProviderEmbeddingsResponse, ProviderError> {
        Err(ProviderError::UnsupportedFeature("NVIDIA does not support embeddings".into()))
    }
...
impl ProviderAdapter for MistralAdapter {
    fn provider_id(&self) -> &'static str { "mistral" }
    fn display_name(&self) -> &'static str { "Mistral AI" }
    fn supports_endpoint(&self, kind: &EndpointKind) -> bool {
        matches!(kind, EndpointKind::ChatCompletions | EndpointKind::ModelList)
    }
    fn auth_kind(&self) -> AuthKind { AuthKind::Bearer }
    fn capabilities(&self) -> ProviderCapabilities {
        ProviderCapabilities {
            openai_compatible: true, has_quirks: false, quirks: vec![],
            supports_streaming: true, supports_tools: true, supports_json_mode: true, supports_vision: true,
            docs_url: Some("https://docs.mistral.ai".into()),
        }
    }
    fn base_url(&self, config: &ProviderConfig) -> Url {
        config.base_url.as_ref().map(|u| u.parse().unwrap()).unwrap_or_else(|| "https://api.mistral.ai/v1".parse().unwrap())
    }
    fn default_headers(&self, config: &ProviderConfig) -> HeaderMap { bearer_auth(config) }
    async fn discover_models(&self, ctx: &ProviderContext) -> Result<Vec<DiscoveredModel>, ProviderError> { shared::openai_discover_models(ctx, "mistral").await }
    async fn chat_completions(&self, ctx: &ProviderContext, request: NormalizedChatRequest) -> Result<ProviderChatResponse, ProviderError> { shared::openai_chat_completions(ctx, request, "mistral").await }
    async fn embeddings(&self, _ctx: &ProviderContext, _request: NormalizedEmbeddingsRequest) -> Result<ProviderEmbeddingsResponse, ProviderError> {
        Err(ProviderError::UnsupportedFeature("Mistral embeddings not implemented".into()))
    }
...
impl ProviderAdapter for ZaiAdapter {
    fn provider_id(&self) -> &'static str { "zai" }
    fn display_name(&self) -> &'static str { "Z AI / Zhipu" }
    fn supports_endpoint(&self, kind: &EndpointKind) -> bool {
        matches!(kind, EndpointKind::ChatCompletions | EndpointKind::ModelList)
    }
    fn auth_kind(&self) -> AuthKind { AuthKind::Bearer }
    fn capabilities(&self) -> ProviderCapabilities {
        ProviderCapabilities {
            openai_compatible: false, has_quirks: true,
            quirks: vec!["Uses /api/paas/v4/ path instead of /v1/".into(), "Extra params: thinking, tool_stream, do_sample".into(), "No /v1/models endpoint".into()],
            supports_streaming: true, supports_tools: true, supports_json_mode: true, supports_vision: true,
            docs_url: Some("https://open.bigmodel.cn/dev/api".into()),
        }
    }
    fn base_url(&self, config: &ProviderConfig) -> Url {
        config.base_url.as_ref().map(|u| u.parse().unwrap()).unwrap_or_else(|| "https://open.bigmodel.cn/api/paas/v4".parse().unwrap())
    }
    fn default_headers(&self, config: &ProviderConfig) -> HeaderMap { bearer_auth(config) }
    async fn discover_models(&self, _ctx: &ProviderContext) -> Result<Vec<DiscoveredModel>, ProviderError> {
        // ZAI has no /models endpoint; return curated catalog
        Ok(vec![
            DiscoveredModel { provider_id: "zai".into(), upstream_model_id: "glm-5.1".into(), display_name: Some("GLM 5.1".into()), endpoint_compatibility: vec!["chat".into()], context_window: Some(200000), free_tier: false },
            DiscoveredModel { provider_id: "zai".into(), upstream_model_id: "glm-4.7-flash".into(), display_name: Some("GLM 4.7 Flash".into()), endpoint_compatibility: vec!["chat".into()], context_window: Some(200000), free_tier: true },
            DiscoveredModel { provider_id: "zai".into(), upstream_model_id: "glm-4-flash-250414".into(), display_name: Some("GLM 4 Flash".into()), endpoint_compatibility: vec!["chat".into()], context_window: Some(128000), free_tier: true },
        ])
    }
    async fn chat_completions(&self, ctx: &ProviderContext, request: NormalizedChatRequest) -> Result<ProviderChatResponse, ProviderError> {
    ...
    async fn embeddings(&self, _ctx: &ProviderContext, _request: NormalizedEmbeddingsRequest) -> Result<ProviderEmbeddingsResponse, ProviderError> {
        Err(ProviderError::UnsupportedFeature("ZAI embeddings not implemented".into()))
    }
...
impl ProviderAdapter for SiliconFlowAdapter {
    fn provider_id(&self) -> &'static str { "siliconflow" }
    ...
    fn base_url(&self, config: &ProviderConfig) -> Url {
        config.base_url.as_ref().map(|u| u.parse().unwrap()).unwrap_or_else(|| "https://api.siliconflow.cn/v1".parse().unwrap())
    }
    fn default_headers(&self, config: &ProviderConfig) -> HeaderMap { bearer_auth(config) }
    async fn discover_models(&self, ctx: &ProviderContext) -> Result<Vec<DiscoveredModel>, ProviderError> { shared::openai_discover_models(ctx, "siliconflow").await }
    async fn chat_completions(&self, ctx: &ProviderContext, request: NormalizedChatRequest) -> Result<ProviderChatResponse, ProviderError> { shared::openai_chat_completions(ctx, request, "siliconflow").await }
    async fn embeddings(&self, _ctx: &ProviderContext, _request: NormalizedEmbeddingsRequest) -> Result<ProviderEmbeddingsResponse, ProviderError> {
        Err(ProviderError::UnsupportedFeature("SiliconFlow embeddings not implemented".into()))
    }
...
impl ProviderAdapter for CloudflareAdapter {
    fn provider_id(&self) -> &'static str { "cloudflare" }
    fn display_name(&self) -> &'static str { "Cloudflare Workers AI" }
    fn supports_endpoint(&self, kind: &EndpointKind) -> bool {
        matches!(kind, EndpointKind::ChatCompletions | EndpointKind::ModelList)
    }
    fn auth_kind(&self) -> AuthKind { AuthKind::Bearer }
    fn capabilities(&self) -> ProviderCapabilities {
        ProviderCapabilities {
            openai_compatible: true, has_quirks: true,
            quirks: vec!["Base URL must include account_id".into(), "Free tier: 10k neurons/day".into(), "Rate limit: 300 req/min for text gen".into()],
            supports_streaming: true, supports_tools: false, supports_json_mode: false, supports_vision: false,
            docs_url: Some("https://developers.cloudflare.com/workers-ai".into()),
        }
    }
    fn base_url(&self, config: &ProviderConfig) -> Url {
        config.base_url.as_ref().map(|u| u.parse().unwrap()).unwrap_or_else(|| "https://api.cloudflare.com/client/v4/accounts/YOUR_ACCOUNT_ID/ai/v1".parse().unwrap())
    }
    fn default_headers(&self, config: &ProviderConfig) -> HeaderMap { bearer_auth(config) }
    async fn discover_models(&self, _ctx: &ProviderContext) -> Result<Vec<DiscoveredModel>, ProviderError> {
    ...
    async fn embeddings(&self, _ctx: &ProviderContext, _request: NormalizedEmbeddingsRequest) -> Result<ProviderEmbeddingsResponse, ProviderError> {
        Err(ProviderError::UnsupportedFeature("Cloudflare embeddings not implemented".into()))
    }
...
impl ProviderAdapter for GitHubModelsAdapter {
    fn provider_id(&self) -> &'static str { "github-models" }
    fn display_name(&self) -> &'static str { "GitHub Models" }
    fn supports_endpoint(&self, kind: &EndpointKind) -> bool {
        matches!(kind, EndpointKind::ChatCompletions | EndpointKind::ModelList)
    }
    fn auth_kind(&self) -> AuthKind { AuthKind::Bearer }
    fn capabilities(&self) -> ProviderCapabilities {
        ProviderCapabilities {
            openai_compatible: true, has_quirks: true,
            quirks: vec!["Uses GitHub PAT (models:read scope) as Bearer token".into(), "Free tier: 15 req/min low, 10 req/min high".into(), "Model format: author/model (e.g. openai/gpt-4o-mini)".into()],
            supports_streaming: true, supports_tools: true, supports_json_mode: true, supports_vision: true,
            docs_url: Some("https://docs.github.com/en/github-models".into()),
        }
    }
    fn base_url(&self, config: &ProviderConfig) -> Url {
        config.base_url.as_ref().map(|u| u.parse().unwrap()).unwrap_or_else(|| "https://models.inference.ai.azure.com".parse().unwrap())
    }
    fn default_headers(&self, config: &ProviderConfig) -> HeaderMap { bearer_auth(config) }
    async fn discover_models(&self, _ctx: &ProviderContext) -> Result<Vec<DiscoveredModel>, ProviderError> {
        Ok(vec![]) // Uses curated catalog for MVP
    ...
    async fn embeddings(&self, _ctx: &ProviderContext, _request: NormalizedEmbeddingsRequest) -> Result<ProviderEmbeddingsResponse, ProviderError> {
        Err(ProviderError::UnsupportedFeature("GitHub Models embeddings not implemented".into()))
    }
...
impl ProviderAdapter for HuggingFaceAdapter {
    fn provider_id(&self) -> &'static str { "huggingface" }
    fn display_name(&self) -> &'static str { "HuggingFace" }
    fn supports_endpoint(&self, kind: &EndpointKind) -> bool {
        matches!(kind, EndpointKind::ChatCompletions | EndpointKind::ModelList)
    }
    fn auth_kind(&self) -> AuthKind { AuthKind::Bearer }
    fn capabilities(&self) -> ProviderCapabilities {
        ProviderCapabilities {
            openai_compatible: true, has_quirks: true,
            quirks: vec!["Rate limit: 1000 req/day free, 20000 req/day PRO".into(), "Model format: author/model-id".into(), "CPU inference for most models".into()],
            supports_streaming: true, supports_tools: false, supports_json_mode: false, supports_vision: false,
            docs_url: Some("https://huggingface.co/docs/api-inference".into()),
        }
    }
    fn base_url(&self, config: &ProviderConfig) -> Url {
        config.base_url.as_ref().map(|u| u.parse().unwrap()).unwrap_or_else(|| "https://api-inference.huggingface.co/v1".parse().unwrap())
    }
    fn default_headers(&self, config: &ProviderConfig) -> HeaderMap { bearer_auth(config) }
    async fn discover_models(&self, _ctx: &ProviderContext) -> Result<Vec<DiscoveredModel>, ProviderError> {
        Ok(vec![]) // Uses curated catalog for MVP
    ...
    async fn embeddings(&self, _ctx: &ProviderContext, _request: NormalizedEmbeddingsRequest) -> Result<ProviderEmbeddingsResponse, ProviderError> {
        Err(ProviderError::UnsupportedFeature("HuggingFace embeddings not implemented".into()))
    }
...
impl ProviderAdapter for CohereAdapter {
    fn provider_id(&self) -> &'static str { "cohere" }
    fn display_name(&self) -> &'static str { "Cohere" }
    fn supports_endpoint(&self, kind: &EndpointKind) -> bool {
        matches!(kind, EndpointKind::ChatCompletions | EndpointKind::ModelList)
    }
    fn auth_kind(&self) -> AuthKind { AuthKind::Bearer }
    fn capabilities(&self) -> ProviderCapabilities {
        ProviderCapabilities {
            openai_compatible: false, has_quirks: true,
            quirks: vec!["Uses /v2/chat endpoint (not /v1/chat/completions)".into(), "Content is array of {type, text} objects".into(), "SSE uses named event types (message-start, content-delta, etc.)".into(), "Free trial: 1000 calls/month, 20 req/min".into()],
            supports_streaming: true, supports_tools: true, supports_json_mode: true, supports_vision: true,
            docs_url: Some("https://docs.cohere.com".into()),
        }
    }
    fn base_url(&self, config: &ProviderConfig) -> Url {
        config.base_url.as_ref().map(|u| u.parse().unwrap()).unwrap_or_else(|| "https://api.cohere.com".parse().unwrap())
    }
    fn default_headers(&self, config: &ProviderConfig) -> HeaderMap { bearer_auth(config) }
    async fn discover_models(&self, _ctx: &ProviderContext) -> Result<Vec<DiscoveredModel>, ProviderError> {
        Ok(vec![
            DiscoveredModel { provider_id: "cohere".into(), upstream_model_id: "command-a-03-2025".into(), display_name: Some("Command A".into()), endpoint_compatibility: vec!["chat".into()], context_window: Some(256000), free_tier: true },
            DiscoveredModel { provider_id: "cohere".into(), upstream_model_id: "command-r7b-12-2024".into(), display_name: Some("Command R7B".into()), endpoint_compatibility: vec!["chat".into()], context_window: Some(128000), free_tier: true },
        ])
    }
    async fn chat_completions(&self, ctx: &ProviderContext, request: NormalizedChatRequest) -> Result<ProviderChatResponse, ProviderError> {
        // Cohere uses /v2/chat with a different format
        let url = ctx.base_url.join("/v2/chat")
        ...
        let text = response_body.get("message")
        ...
        let finish_reason = response_body.get("finish_reason")
            .and_then(|r| r.as_str())
            .map(|r| match r {
                "COMPLETE" => "stop",
                "MAX_TOKENS" => "length",
    ...
    async fn embeddings(&self, _ctx: &ProviderContext, _request: NormalizedEmbeddingsRequest) -> Result<ProviderEmbeddingsResponse, ProviderError> {
        Err(ProviderError::UnsupportedFeature("Cohere embeddings not implemented".into()))
    }
